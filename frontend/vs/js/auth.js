...
export function isTokenValid() {
  ...
  return Date.now() < accessTokenExpiry - EXPIRY_MARGIN_MS;
}

/** Принудительно обновить Samokat-токен. Возвращает true если успешно. */
...
export async function tryRestoreSession() {
  ...
  if (!storedRefresh) {
    // vs-сессия есть, но Samokat-токена нет — пускаем, WMS-функции будут недоступны
    notifyChange(true);
    return true;
  }
  ...
  if (!ok) {
    // Рефреш упал, но vs-сессия валидна — не выбрасываем, ретраим в фоне
...
function scheduleRefresh() {
  ...
  refreshTimer = setTimeout(async () => {
    const ok = await doRefresh();
    if (!ok) {
      // Не выбрасываем пользователя — vs-сессия остаётся валидной.
      // Ретраим с экспоненциальной задержкой (30с → 1м → 2м → ... → 10м макс.)
      _retryCount = (_retryCount || 0) + 1;
      const retryDelay = Math.min(_retryCount * 30000, 10 * 60 * 1000);
      refreshTimer = setTimeout(async () => {
        const ok2 = await doRefresh();
        if (ok2) _retryCount = 0;
