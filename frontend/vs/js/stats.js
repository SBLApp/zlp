...
import { getProductWeights } from './api.js';

/** Кэш весов из Excel: { [article]: grams }. Загружается при инициализации модуля. */
let _productWeights = {};
getProductWeights().then(w => { _productWeights = w; });

/** Получить вес в граммах: Excel (по артикулу) → парсинг имени → 0. */
...
export function renderHourlyByEmployee(items, shiftFilter = 'day', emplMap = null, showIdles = false, idlesByEmployee = {}, allowedIdleMinutes = 0, mode = 'hourly', shiftMinutes = 12 * 60, thresholdMinutes = 15) {
  ...
  for (const item of items || []) {
    ...
    const name = item.productName || item.product || item.name;
    if (!name) continue;
    const art2 = String(item.nomenclatureCode || item.article || '').trim();
    const gramsPerUnit = resolveWeightGrams(art2, name);
    if (gramsPerUnit <= 0) continue;
    const qty = Math.max(1, Number(item.quantity) || 1);
    const grams = gramsPerUnit * qty;
