...
/** Результаты точечных запросов (нормФИО → timestamp). Заполняется в refreshMonitor. */
let lastCompletedAtFromQueries = new Map();

/** Накопленный кеш executorId (нормФИО → uuid). Пополняется из live и статистики, сохраняется в localStorage. */
const EXECUTOR_ID_CACHE_KEY = 'monitorExecutorIdCache';
const executorIdCache = (() => {
  const map = new Map();
  try {
    const saved = localStorage.getItem(EXECUTOR_ID_CACHE_KEY);
    if (saved) for (const [k, v] of Object.entries(JSON.parse(saved))) map.set(k, v);
  } catch {}
  return map;
})();
function saveExecutorIdCache() {
  try { localStorage.setItem(EXECUTOR_ID_CACHE_KEY, JSON.stringify(Object.fromEntries(executorIdCache))); } catch {}
}
...
/** Пополняет кеш executorId из массива items (вызывать при каждой загрузке allItems). */
export function updateExecutorIdCacheFromItems(items) {
  let changed = false;
  for (const item of items || []) {
    if (!item.executorId) continue;
    const norm = normalizeFio(item.executor);
    if (norm && !executorIdCache.has(norm)) { executorIdCache.set(norm, item.executorId); changed = true; }
  }
  if (changed) saveExecutorIdCache();
}
...
export function getRollcallPresent() {
  return rollcallPresent;
}

/** Имена исполнителей из JSON-файлов текущей даты (обновляется из app.js при загрузке summary) */
let _todayExecutorNames = [];

export function setTodayExecutorNames(names) {
  _todayExecutorNames = Array.isArray(names) ? names : [];
}

export function startMonitorRefresh() {
  if (monitorInterval) clearInterval(monitorInterval);
  monitorInterval = setInterval(() => refreshMonitor(), 3 * 60 * 1000);
}
...
export async function refreshMonitor() {
  ...
  let snapshot;
  try {
    // Если токен устарел — обновляем перед запросом
    if (!auth.isTokenValid() && auth.getToken()) {
  ...
  if (token && rollcallPresent.size > 0) {
    // Пополняем кеш из live-снапшота
    let cacheChanged = false;
    for (const [norm, entry] of snapshot) {
      if (entry.userId && !executorIdCache.has(norm)) { executorIdCache.set(norm, entry.userId); cacheChanged = true; }
    }
    // Пополняем кеш из загруженных операций
    const items = _getItemsFn();
    for (const item of items) {
      if (!item.executorId) continue;
      const norm = normalizeFio(item.executor);
      if (!executorIdCache.has(norm)) { executorIdCache.set(norm, item.executorId); cacheChanged = true; }
    }
    if (cacheChanged) saveExecutorIdCache();
    const now = new Date();
    const toIso = now.toISOString();
    // Начало текущей смены: день 09:00, ночь 21:00 предыдущего дня
    const h = now.getHours();
    let shiftStart;
    if (h >= 9 && h < 21) {
      shiftStart = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 9, 0, 0, 0);
    } else {
      shiftStart = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 21, 0, 0, 0);
      if (h < 9) shiftStart.setDate(shiftStart.getDate() - 1);
    }
    const fromShift = shiftStart.toISOString();

    // Строим lastCompletedAtMap из загруженных операций — нужен для определения приоритета
    const itemsForSort = _getItemsFn();
    ...
    // Сохраняем предыдущий раунд запросов для сортировки, потом сбрасываем
    const prevQueries = new Map(lastCompletedAtFromQueries);
    lastCompletedAtFromQueries = new Map();

    // Приоритет: сначала те, у кого совсем нет данных (ни из операций, ни из прошлого раунда запросов)
    ...
    const hasDataGroup = [];
    for (const normFio of rollcallPresent) {
      if (findByPersonKey(executorIdCache, normFio)) {
        (hasAnyData(normFio) ? hasDataGroup : noDataGroup).push(normFio);
      }
    }

    // Сначала запрашиваем тех, по кому нет данных
    for (const normFio of noDataGroup) {
      const executorId = findByPersonKey(executorIdCache, normFio);
      try {
      ...
      } catch { /* один сбой — не ломаем весь мониторинг */ }
    }

    // Промежуточный рендер — показываем результат по «без данных» как можно раньше
...
function renderEmployeeRow(r) {
  ...
  // Тип задачи из live — только как доп. инфо, не как критерий
  const taskTypeBadge = r.taskType
    ? `<span class="mon-task-type mon-task-type--${(r.taskType||'').toLowerCase()}">${escH(r.taskType)}</span>`
    : '—';

  if (r.inWorkByTask) {
    // В работе по статистике (последний пик ≤ 10 мин)
    const taskStr = r.taskDurationMs ? formatDuration(r.taskDurationMs) : '—';
    const durationHint = r.isActive && r.taskDurationMs
      ? ` <span class="mon-idle-hint">(задача ${taskStr})</span>`
      : '';
    return `
      <tr class="mon-row--active mon-row-clickable" data-norm-fio="${escH(r.normFio)}" title="Нажмите, чтобы изменить компанию и статус на смене">
        ...
        <td class="mon-td-type">${taskTypeBadge}</td>
        <td class="mon-td-status">🟢 в работе ${wasAbsent}</td>
        <td class="mon-td-idle"><span class="mon-idle-time mon-idle-time--work">${lastTaskStr} назад</span>${durationHint}</td>
        <td class="mon-td-sz">${szVal}</td>
      </tr>`;
  } else {
    // Не в работе: последний пик > 10 мин или данных нет
    ...
    return `
      <tr class="mon-row--inactive mon-row-clickable" data-norm-fio="${escH(r.normFio)}" title="Нажмите, чтобы изменить компанию и статус на смене">
        <td class="mon-td-name">${escH(r.displayFio)}</td>
        <td class="mon-td-type">${taskTypeBadge}</td>
        <td class="mon-td-status">🔴 не в работе</td>
        <td class="mon-td-idle"><span class="mon-idle-time mon-idle-time--idle">${sinceStr}</span></td>
        <td class="mon-td-sz">${szVal}</td>
      </tr>`;
  }
}
...
function renderRollcallModal() {
  ...
  // Собираем все ФИО из empl + live + уже отмеченных в перекличке
  ...
  // Люди, добавленные вручную через «Добавить» — могут не быть в emplMap и live
  for (const normFio of rollcallPresent) {
    if (!allFios.has(normFio)) {
      // Ищем красивое отображение в _todayExecutorNames или оставляем titleCase
      const displayFio = _todayExecutorNames.find(n => normalizeFio(n) === normFio) || titleCase(normFio);
  ...
  // Менеджер видит только свои компании
  if (auth.getRole() === 'manager') {
    const allowed = auth.getCompanyIds();
    if (allowed && allowed.length > 0) {
      const allowedSet = new Set(allowed.map(c => String(c).trim().toLowerCase()));
...
/** Возвращает HTML-блок с людьми из live и статистики, отсутствующими в перекличке */
function renderSuggestionsSection() {
  ...
  // Из загруженных операций (allItems) или из JSON-сводки (executor names)
  const statItems = _getItemsFn();
  const executorSource = statItems.length > 0
    ? statItems.map(i => i.executor).filter(Boolean)
    : _todayExecutorNames;
  ...
  for (const executor of executorSource) {
    ...
    seenPKs.add(pk);
    const company = findByPersonKey(_emplMap, normFio) || '—';
    suggestions.push({ normFio, displayFio: executor, company, taskType: null });
  }
