...
import { initShipments, loadSummary as loadShipmentsSummary } from './shipments.js';
...
let lastKnownRun = null;       // последний lastRun, замеченный в loadStatus
let summaryLastRun = null;     // lastRun в момент последней загрузки summary
let autoFetchEnabled = false;
try { autoFetchEnabled = localStorage.getItem('vs_auto_fetch_enabled') === '1'; } catch { /* ignore */ }
let autoFetchInProgress = false;
/** Ссылка на runFetchForHours, устанавливается внутри setupEventListeners для доступа снаружи */
let _runFetchForHours = null;
...
async function init() {
  ...
  // Статус планировщика — каждые 10 сек
...
const LS_ACTIVE_TAB = 'vs_active_tab';
...
function applyModulesVisibility() {
  ...
  let savedTab = null;
  try { savedTab = localStorage.getItem(LS_ACTIVE_TAB); } catch { /* ignore */ }
  const savedBtn = savedTab ? document.querySelector(`.tab-btn[data-tab="${savedTab}"]`) : null;
  const savedMod = savedBtn?.dataset.module;

  if (savedBtn && savedMod && allowed.has(savedMod)) {
    switchTab(savedTab);
  } else {
    const activeBtn = document.querySelector('.tab-btn.active');
    const activeMod = activeBtn?.dataset.module;
    if (activeBtn && (!activeMod || !allowed.has(activeMod))) {
      const pick = Array.from(document.querySelectorAll('.tab-btn[data-module]')).find(b => allowed.has(b.dataset.module));
...
function switchTab(tabId) {
  ...
  try { localStorage.setItem(LS_ACTIVE_TAB, tabId); } catch { /* ignore */ }
  ...
  // При открытии отгрузки — грузим сводку
  if (tabId === 'tab-shipments') {
...
function syncDatePickers() {
  for (const id of ['date-picker-stats']) {
    const input = el(id);
...
function syncShiftToggle() {
  for (const id of ['shift-toggle-day-stats', 'shift-toggle-night-stats']) {
    const btn = el(id);
    if (!btn) continue;
...
async function loadDateSummary(dateStr) {
  ...
  try {
    ...
    summaryLastRun = lastKnownRun; // фиксируем: сводка соответствует этому lastRun
...
function startAutoFetch() {
  ...
  autoFetchTimer = setInterval(() => doAutoFetch(), 3 * 60 * 1000);
}

async function doAutoFetch() {
  if (selectedDate !== getTodayStr()) return;
  if (autoFetchInProgress) return;
  if (!_runFetchForHours) return;
  // Всегда обновляем токен перед фетчем — не ждём пока он протухнет
  const refreshed = await auth.forceRefresh();
  if (!refreshed) return;
  autoFetchInProgress = true;
  try {
    await _runFetchForHours(false, true);
    // Весь фетч завершён — только теперь сообщаем серверу об обновлении
...
function updateAutoFetchIndicator() {
  const indicator = el('schedule-indicator');
  if (!indicator) return;
  if (autoFetchEnabled) {
    indicator.className = 'schedule-dot dot-green';
    indicator.title = 'Автообновление включено на этом устройстве';
  } else {
    indicator.className = 'schedule-dot dot-gray';
    indicator.title = 'Автообновление выключено';
  }
}

function updateRequestFetchBtnVisibility() {
  const btn = el('btn-request-fetch');
  if (!btn) return;
  btn.style.display = autoFetchEnabled ? 'none' : '';
}
...
async function loadStatus() {
  try {
    ...
    if (lastRunStats) lastRunStats.textContent = lastRunText || 'Ожидание обновления...';

    // Фиксируем последний известный lastRun
    if (status.lastRun && status.lastRun !== lastKnownRun) {
      lastKnownRun = status.lastRun;
    }

    // Если сводка устарела — тихо обновляем (для всех клиентов)
    if (lastKnownRun && lastKnownRun !== summaryLastRun && selectedDate === getTodayStr()) {
      loadDateSummary(selectedDate);
    }

    // Удалённый запрос на фетч: если флаг стоит и мы корп. устройство
    if (status.fetchRequested && autoFetchEnabled && !autoFetchInProgress) {
      triggerImmediateFetch();
    }
  } catch { /* ignore */ }
}

function showDataAvailableBanner(lastRun) {
  const banner = el('data-available-banner');
  const text = el('data-available-text');
  if (!banner) return;
  if (text) text.textContent = 'Данные обновлены ' + formatDateTime(lastRun) + ' — нажмите чтобы увидеть';
...
function renderTableLoadDataPlaceholder() {
  // вкладка «Данные» удалена — ничего не делаем
}
...
const VS_MODULE_LABELS = { stats: 'Статистика', data: 'Данные', monitor: 'Мониторинг', analysis: 'Анализ', consolidation: 'Консолидация', docs: 'Документы', settings: 'Настройки', shipments: 'Отгрузка' };
...
function initVsUserEditModalCheckboxes() {
  ...
  const modules = ['stats', 'data', 'monitor', 'analysis', 'consolidation', 'docs', 'settings', 'shipments'];
...
/** Список «Сотрудники без компании»: из статистики/данных, но не в empl.csv. Bubble-стиль. */
function renderEmplNoCompanyList(emplMapArg) {
  ...
  // Из summary-данных (работает без вкладки «Данные»)
  ...
  const noCompany = [...fioToFull.values()].sort((a, b) => a.localeCompare(b, 'ru'));

  if (emptyEl) emptyEl.style.display = noCompany.length ? 'none' : 'block';
  listEl.innerHTML = noCompany.map(fio => {
    const parts = fio.trim().split(/\s+/);
    const partsHtml = parts.map(p => `<span class="empl-bubble-part">${escAttr(p)}</span>`).join('');
    return `<li><button type="button" class="btn-empl-fio" data-fio="${escAttr(fio)}">${partsHtml}</button></li>`;
  }).join('');
...
function setupEventListeners() {
  ...
  // Выбор даты. Загружаем только сводку.
  for (const id of ['date-picker-stats']) {
  ...
  async function runFetchForHours(forceRecheck, silent = false) {
    ...
    if (!silent) showNotification(forceRecheck
    ...
    if (!silent) showNotification(`Получено ${res.fetched}, добавлено ${res.added}`, 'success');
    ...
    await loadDateSummary(selectedDate);
    const text = 'Обновлено: ' + formatDateTime(new Date().toISOString());
    const lrEl = el('last-run'); const lrsEl = el('last-run-stats');
    if (lrEl) lrEl.textContent = text;
    if (lrsEl) lrsEl.textContent = text;
    await loadStatus();
  }
  // Expose runFetchForHours to module scope so triggerImmediateFetch can use it
  _runFetchForHours = runFetchForHours;
  ...
  // Настройки: toggle «Все сотрудники»
  el('btn-empl-editor-toggle')?.addEventListener('click', () => {
    const toggle = el('btn-empl-editor-toggle');
    const body = el('empl-editor-body');
    const isOpen = body?.classList.toggle('open');
    toggle?.classList.toggle('open', isOpen);
  });

  // Настройки: добавить пустую строку — открываем список
  el('btn-add-empl-row')?.addEventListener('click', () => {
    el('empl-editor-body')?.classList.add('open');
    el('btn-empl-editor-toggle')?.classList.add('open');
    const tbody = el('empl-editor-tbody');
    if (!tbody) return;
  ...
  // Поиск по сотрудникам в настройках — открываем список при вводе
  el('empl-search-input')?.addEventListener('input', () => {
    if (el('empl-search-input').value) {
      el('empl-editor-body')?.classList.add('open');
      el('btn-empl-editor-toggle')?.classList.add('open');
    }
    filterEmplSearch();
  ...
  const autoFetchToggle = el('setting-auto-fetch');
  if (autoFetchToggle) {
    autoFetchToggle.checked = autoFetchEnabled;
    autoFetchToggle.addEventListener('change', () => {
      autoFetchEnabled = autoFetchToggle.checked;
      try { localStorage.setItem('vs_auto_fetch_enabled', autoFetchEnabled ? '1' : '0'); } catch { /* ignore */ }
      if (autoFetchEnabled) startAutoFetch();
      else stopAutoFetch();
      updateAutoFetchIndicator();
      updateRequestFetchBtnVisibility();
    });
  }
  if (autoFetchEnabled) startAutoFetch();
  updateAutoFetchIndicator();
  ...
  // Баннер "Данные обновлены"
  el('btn-apply-update')?.addEventListener('click', () => {
    const banner = el('data-available-banner');
  ...
  el('btn-request-fetch')?.addEventListener('click', async () => {
    await api.requestFetch();
    showNotification('Запрос на обновление отправлен', 'info');
  });
}
