...
# --- 2.1. .NET зависимости (ускорение save-fetched-data) ---
if command -v dotnet >/dev/null 2>&1; then
  echo "[*] .NET найден: $(dotnet --version)"
  echo "[*] Восстановление .NET зависимостей..."
  dotnet restore tools/SaveFetchedData/SaveFetchedData.csproj >/dev/null
  dotnet restore tools/WeightScan/WeightScan.csproj >/dev/null
  echo "[*] Сборка .NET (Release)..."
  dotnet build tools/SaveFetchedData/SaveFetchedData.csproj -c Release >/dev/null
  echo "[OK] .NET инструменты готовы."
else
  echo "[!] dotnet не найден. Ускорение save-fetched-data будет отключено."
fi
