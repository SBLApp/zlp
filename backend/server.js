...
// GET /api/missing-weight — список товаров без веса (фильтруем те, что уже есть в Excel)
app.get('/api/missing-weight', (req, res) => {
  const list = loadMissingWeight().filter(item => {
    const article = String(item.article || '').trim();
    return !article || productWeights.getWeightGrams(article) <= 0;
  });
  res.json(list);
});
...
app.get('/api/date/:date/summary', vsSessionOptional, (req, res) => {
  try {
    ...
    const summary = storage.getDateSummary(dateStr, { shift, idleThresholdMs }, { getCompany });
...
app.get('/api/shipments/report', vsSessionRequired, async (req, res) => {
  try {
    ...
    const filename = encodeURIComponent('Отчет по РК СПБ-3.xlsx');
...
app.post('/api/rk/routes/:routeId/ship', (req, res) => {
  try {
    ...
    const { by, gate, items, photos } = req.body;
    if (!Array.isArray(items) || !items.length) {
      return res.status(400).json({ ok: false, error: 'Не указаны позиции ЦФЗ' });
    }
    const route = rkStorage.submitShipment(routeId, { by, gate, items, photos });
...
app.post('/api/rk/routes/:routeId/receive', (req, res) => {
  try {
    ...
    const route = rkStorage.submitReceiving(routeId, { by, gate, items, photos });
    sseNotify('routes-updated');
    res.json({ ok: true, route });
  } catch (err) {
    res.status(400).json({ ok: false, error: err.message });
...
app.post('/api/rk/routes/:routeId/confirm-ship', vsSessionRequired, (req, res) => {
  try {
    const route = rkStorage.confirmShipment(decodeURIComponent(req.params.routeId), req.vsSession?.login);
    res.json({ ok: true, route });
  } catch (err) {
    res.status(400).json({ ok: false, error: err.message });
  }
...
app.post('/api/rk/routes/:routeId/confirm-receive', vsSessionRequired, (req, res) => {
  try {
    const route = rkStorage.confirmReceiving(decodeURIComponent(req.params.routeId), req.vsSession?.login);
    res.json({ ok: true, route });
  } catch (err) {
    res.status(400).json({ ok: false, error: err.message });
  }
...
const RECEIVE_DIR = path.join(FRONTEND_DIR, 'receive');
app.get('/receive', (req, res) => {
  res.sendFile(path.join(RECEIVE_DIR, 'index.html'));
});
app.use('/receive', express.static(RECEIVE_DIR));
