...
// GET /api/missing-weight — список товаров без веса
...
app.post('/api/missing-weight/sync', (req, res) => {
  try {
    ...
    // Удаляем те, у которых появился вес
...
function readEmplCsvText() {
  ...
  // Проверка: если первые байты похожи на UTF-8 Cyrillic (D0/D1 xx)
  const iconv = require('iconv-lite');
  const utf8text = buf.toString('utf8');
  if (/[\u0400-\u04FF]/.test(utf8text)) return utf8text;
